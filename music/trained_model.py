####NOTE: This is included so that the code runs, but is copied from Magenta's implementation of MusicVAE with very minimal changes.  
...
class TrainedModel(object):
  ...
  def encode(self, note_sequences, assert_same_length=False):
    ...
    for note_sequence in note_sequences:
      extracted_tensors = self._config.data_converter
      extracted_tensors = extracted_tensors.to_tensors(note_sequence)
  ...
  def decode(self, z, length=None, temperature=1.0, c_input=None, name = None):
...
utput_depth,
      ...
      self._temperature = tf.placeholder(tf.float32, shape=())
      self._z_input = tf.placeholder(tf.float32, shape=[batch_size, self._config.hparams.z_size]) if self._config.hparams.z_size else None
      self._c_input = tf.placeholder(tf.float32, shape=[None, self._config.data_converter.control_depth]) if self._config.data_converter.control_depth > 0 else None
      self._inputs = tf.placeholder(tf.float32, shape=[batch_size, None, self._config.data_converter.input_depth])
      self._controls = tf.placeholder(tf.float32, shape=[batch_size, None, self._config.data_converter.control_depth])
      self._inputs_length = tf.placeholder(tf.int32, shape=[batch_size] + list(self._config.data_converter.length_shape))
      self._max_length = tf.placeholder(tf.int32, shape=())
      ...
      # Variable renaming for backwards compatibility
      var_map = {v.name[:-2]: v for v in tf.global_variables()} if var_name_substitutions is None else {
        re.sub(pattern, substitution, v.name[:-2]): v
        for v in tf.global_variables()
        for pattern, substitution in var_name_substitutions
      }
  ...
  def _restore_checkpoint(self, saver, checkpoint_path):
    """
    Helper function to restore a model from a checkpoint.
    """
    if (os.path.exists(checkpoint_path) and tarfile.is_tarfile(checkpoint_path)):
      ...
      with tempfile.TemporaryDirectory() as temp_dir:
        ...
        checkpoint_path = next(
            os.path.join(temp_dir, name[:-6])
            for name in tar.getnames() if name.endswith('.index')
        )
      saver.restore(self._sess, checkpoint_path)
    else:
      saver.restore(self._sess, checkpoint_path)
